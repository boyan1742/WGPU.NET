...
namespace WGPU.NET
{
    public class WgpuStructChain : IDisposable
    {
        ...
        public WgpuStructChain AddInstanceExtras(Wgpu.InstanceBackend backend, Wgpu.InstanceFlag flags, Wgpu.Dx12Compiler dx12Compiler, Wgpu.Gles3MinorVersion gles3MinorVersion,
            string dxilPath, string dxcPath)
        {
            AddStruct(new Wgpu.InstanceExtras()
            {
                chain = new Wgpu.ChainedStruct() { sType = (Wgpu.SType)Wgpu.NativeSType.STypeInstanceExtras },
                backends = (uint)backend,
                flags = (uint)flags,
